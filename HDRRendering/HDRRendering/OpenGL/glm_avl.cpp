...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    char* t_filename;
    char    buf[128];
    GLuint nummaterials, numtextures, i;
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        //AVL hit a texture description 
        case 'm':
            fgets(buf, sizeof(buf), file);
            numtextures++;
            
            sscanf(buf, "%s %s", buf, buf);
        //AVL End texture desciption
            break;
        default:
            /* eat up rest of line */
            fgets(buf, sizeof(buf), file);
    ...
    for (i = 0; i < nummaterials; i++) {
        ...
        //AVL texture addition code below
        model->materials[i].image = NULL;
		model->materials[i].textured = 0;
        model->materials[i].t_filename = NULL;
        model->materials[i].width = 0; 
        model->materials[i].height = 0;
        model->materials[i].t_id[0] = 0;    
        //AVL END texture addition code below
    }
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'n':               /* newmtl */
			...
			model->materials[nummaterials].textured = 0;
            break;
        case 'N':
        ...
        case 'K':
            ...
            break;
        case 'd': //AVL Opacity case
            float alpha;
            fscanf(file, "%f", &alpha);
            model->materials[nummaterials].diffuse[3] = alpha; 
            if (alpha < 1.0)
                GLM_BLENDING = 1;
            break;// AVL End Opacity Case   
        
        case 'm': //AVL Texture Grab and setup
            fgets(buf, sizeof(buf), file);
            sscanf(buf, "%s %s", buf, buf);
            numtextures++;
            t_filename = (char*)malloc(sizeof(char) * (strlen(dir) + strlen(buf) + 1));
            strcpy(t_filename, dir);
            strcat(t_filename, buf);
            model->materials[nummaterials].t_filename = strdup(t_filename);
            
       		glGenTextures(1, model->materials[nummaterials].t_id);

            
            //AVL DEBUG printf ("\n%s", model->materials[nummaterials].t_name);
            model->materials[nummaterials].image = glmReadPPM(model->materials[nummaterials].t_filename,
                                                                      &model->materials[nummaterials].width,
                                                                      &model->materials[nummaterials].height);

                            
            if(model->materials[nummaterials].image)
            {
            glmFlipTexture(model->materials[nummaterials].image,
                           model->materials[nummaterials].width,
                           model->materials[nummaterials].height);
          	glBindTexture(GL_TEXTURE_2D, model->materials[nummaterials].t_id[0]);
            glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
            gluBuild2DMipmaps(GL_TEXTURE_2D, 3, model->materials[nummaterials].width, model->materials[nummaterials].height,
                              GL_RGB, GL_UNSIGNED_BYTE,  model->materials[nummaterials].image);
            }
			model->materials[nummaterials].textured = 1;
            free(t_filename);
            break;//AVL END Texture Grab and setup
        default:
            /* eat up rest of line */
            fgets(buf, sizeof(buf), file);
            break;
...
glmDelete(GLMmodel* model)
{
    ...
    if (model->materials) {
        for (i = 0; i < model->nummaterials; i++)
        {
            glDeleteTextures(1, model->materials[i].t_id);
            
            free(model->materials[i].t_filename);
            
            free(model->materials[i].image);
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    //AVL TWO SIDED Toggle
    if(mode & GLM_2_SIDED)
        glLightModeli(GL_LIGHT_MODEL_TWO_SIDE, GL_TRUE); 
    else
        glLightModeli(GL_LIGHT_MODEL_TWO_SIDE, GL_FALSE); 
    //AVL END TWO SIDED Toggle
    ...
    //AVL CHEESY BLENDING (AKA: NO SORTING)
    if(GLM_BLENDING) //If model has blending, use two pass approach, alpha items last
    {
     //AVL DEBUG printf ("\nUsing Double Pass");
    int blending = 0; //AVL blending tag

    group = model->groups; //First pass for non alpha items
    while (group) {
        ...
        if (mode & GLM_MATERIAL) {
            ...
            if(material->diffuse[3] < 1.0) //AVL Opacity Check followed by CHEESY BLENDING (AKA: NO SORTING)
                {
                blending = 1;
                }                          //END AVL Opacity check
            }
        if (!blending) //AVL If Model has no blending 
        {
            //AVL Texture Binding
            if (mode & GLM_TEXTURE && material->textured) {
           	    glBindTexture(GL_TEXTURE_2D, material->t_id[0]);  
            }   
			else
			{
				glBindTexture(GL_TEXTURE_2D, NULL);
			}
            //AVL END Texture Binding
            ...
            for (i = 0; i < group->numtriangles; i++) {
                ...
                if ((mode & GLM_TEXTURE) && (mode & GLM_MATERIAL) && material->textured) //AVL Null Texture check added
        ...
        blending = 0; //AVL Reset Blending Tag
        group = group->next;
    }

    //AVL Prep for sceond pass with alpha items
    ...
    while (group) {
        ...
        if (mode & GLM_MATERIAL) {
            ...
            glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, material->specular);
            glMaterialf(GL_FRONT_AND_BACK, GL_SHININESS, material->shininess);
            if(material->diffuse[3] < 1.0) //AVL Opacity Check floowed by CHESSY BLENDING - NO SORTING
                {
                blending = 1;
                }                          //END AVL Opacity check
            }
        if (blending) //If blending occurs at current group, draw it
        {
            //AVL Texture Binding
            if (mode & GLM_TEXTURE && material->textured) {
           	    glBindTexture(GL_TEXTURE_2D, material->t_id[0]);  
    ...
    else //Else no blending used in this model, so ignore it.
    {
    //AVL DEBUG printf ("\nUsing Single Pass");
    group = model->groups;
    while (group) {
...
GLvoid glmFlipModelTextures(GLMmodel* model)
{
    ...
    while (group) {
        ...
        //AVL Texture Flip
        if (material->image && material->textured) {
