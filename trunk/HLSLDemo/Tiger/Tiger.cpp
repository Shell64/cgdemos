...
D3DXHANDLE WorldHandle = 0;
D3DXHANDLE ViewHandle = 0;
D3DXHANDLE ProjectHandle = 0;
...
bool Setup()
{
	...
	WorldHandle = VSEffect->GetParameterByName(0, "World");
	ViewHandle = VSEffect->GetParameterByName(0, "View");
	ProjectHandle = VSEffect->GetParameterByName(0, "Projection");
...
bool Display(float timeDelta)
{
	if( Device ) // Only use Device methods if we have a valid device.
	{
		...
		VSEffect->SetMatrix(WorldHandle, &W);		
		VSEffect->SetMatrix(ViewHandle, &V);
		VSEffect->SetMatrix(ProjectHandle, &P);		

		// Instruct the device to set each pixel on the back buffer black -
		// D3DCLEAR_TARGET: 0x00000000 (black) - and to set each pixel on
		// the depth buffer to a value of 1.0 - D3DCLEAR_ZBUFFER: 1.0f.
		Device->Clear(0, 0, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, 0xff202080, 1.0f, 0);
